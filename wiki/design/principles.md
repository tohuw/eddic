...
The DM wiki is the single maintained master. Visibility is frontmatter, and it fails closed: pages are DM-only unless marked `visibility: player`. The player wiki is a deterministic build-time *projection* — revealing a page to players is "lifting the veil," one frontmatter change. A firewall lint proves no player-visible page links to a DM-only page, and player-facing surfaces (sites, bots, connectors) ingest only the projection, by construction. No agent decides what leaks *through a link*: a build script decides and a lint audits, and that half is a real guarantee. Prose is the half no structural check reaches — a secret told in a sentence on a player-visible page breaks nothing the firewall can see — so the semantic pass exists to read for it, advisory by nature and genuinely load-bearing: it has caught a live prose leak the structural check could not. Claiming more than that for the firewall would be claiming the wrong thing on the one promise this project cannot afford to overstate. The mechanics live in [The firewall](../concepts/the-firewall.md), [Projection and visibility](../concepts/projection-and-visibility.md), and the [lint](../modules/lint.md) module.
...
### 11. Who holds the pen

Every surface where words enter a campaign has a pen-holder, and it is a marked per-surface choice, never a global stance. The interviewer's scribe/drafter dial, the mode a source is ingested under, whether a recap is the agent's prose or the DM's own, a page's curation mark, and a page's advisory-lint lock are one question in five costumes: *whose expression is this?* Each surface answers separately, every answer is frontmatter rather than a vibe, and each default sits where the honest answer sits — agent prose where an agent wrote it, the human's words where a human wrote them. This is what makes easing in mechanical: a table that wants the machine writing nothing starts with every pen human and moves them one surface at a time as trust accrues; a table that wants everything drafted starts at the other end. Principle 6 governs the past — authorship records who wrote a thing and is never adjusted to flatter a preference. This principle governs the future: who writes it next. The positions are developed in [Who holds the pen](#who-holds-the-pen) below.

### 12. Personality lives at owned surfaces, never in the data layer
...
Principle 11 in its working form. Five surfaces decide who writes the campaign's words, each marked per page or per run, each defaulting to the honest answer rather than the cautious one:

| surface | dial | default |
|---|---|---|
| interview | `scribe` (the writer's own words, cleaned mechanically) / `drafter` (agent prose from interview notes) | scribe |
| ingest | `literal` (spelling, grammar, linkification — nothing else) / `derived` (the agent renders sources into wiki prose) | derived for pages the agent compiles, literal for anything a human wrote |
| recap | machine-authored from the transcript / the DM's own write-up | machine |
| curation | `agent` / `human` — who is answerable for the page as it now stands | agent |
| advisory lint | on / `lint: off` per page | on |

Three things this doctrine refuses to blur:

- **Curation is not authorship.** `authorship:` records who produced the expression — `machine`, `mixed`, `human`, `transcript`, or a contributor id — and it is a fact about the past, not a setting. `curation:` records who is answerable for the page now. A human who reads a machine-written page and approves it has curated it; they have not written it, and no agent ever rewrites `authorship: machine` to `human` because someone blessed the page. That edit would assert protection the law does not grant to machine output (the rights posture above) — a lie pointed in the legally dangerous direction, told about the one seam a rights question actually examines. What *does* move authorship is substantive human revision: interview the writer, let them rework the prose in their own words, and the page becomes `mixed` honestly. A rewrite, never a checkbox. For the same reason curation is asserted and never inferred: it does not default from authorship, because inferring one axis from the other is the conflation this rule exists to prevent.
- **Marks are assertions, not detections.** Nothing in Eddic tries to detect machine prose, and nothing should. Authorship and curation are sworn by the people who set them and enforced by friction and honesty — a detector would be wrong often enough to be worse than useless, and it would put a machine's guess ahead of a human's word about their own writing.
- **Quiet is not silent, and never unguarded.** Where the pen is held elsewhere the linter stops nagging: advisory checks go quiet under `curation: human`, under the owner's `lint: off`, and on unadjudicated merge proposals. The firewall, the rights and attribution machinery, and structural integrity are never quietable, by any marker, on any page — a check is binding unless deliberately marked advisory, so the tier fails closed like the visibility axis it protects. Quieted findings are still counted and still listable; the owner opting out of advice never opts out of the safety net.

The payoff is that easing in becomes mechanical rather than a matter of trust. A table that wants the machine writing nothing starts with every pen human and moves them one surface at a time as confidence accrues; a table that wants everything drafted starts at the other end and keeps only the surfaces it cares about. Neither position is a fork of Eddic, a setting buried in config, or a conversation — it is five markers, and the wiki says at every page which way each is turned.

## Companions at the table (future module family)

In-session agents for players and the DM, each side seeing its own retrieval tier.

Two different harms hide under one slogan, and they deserve different instruments. **Unearned knowledge** — a player learning what their character could not — is a real harm and is handled *structurally*: the player tier physically cannot see DM material, so a player companion cannot leak a secret it was never given. That guarantee owes nothing to the companion's manners. **Tactical help** — telling a player what play is better — is not a secret at all. It is a question about whose game it is, and it has no technical answer, because the player can ask any chatbot the same question in the next tab and get a ranked answer immediately.

So the conduct rule — **the agent may tell you what is possible and what is true; it may never tell you what is better** — is a strong *default*, not a doctrine. It is right for shared, in-session surfaces, where the table has not agreed to an optimizer at its game and one player's ranked answer spends everyone else's spotlight. It is wrong applied absolutely: a table that wants tactical help should be able to have it, and a player asking how their own character works, outside a scene, was never taking anything from anybody. The one place the old absolutism cost most for least was the learner's primer, where the person least able to play was the person least helped. Where the rule is on, "what should I do?" still gets the option landscape and the reminder that in D&D you can attempt almost anything; it does not get a pick.

Per no-egg-sucking, the rule ships as a short affirmative standing instruction, not an enumeration of cases. The abuse backstop is social, not technical, and patterns say so plainly: Eddic is vendored plaintext anyone can rewrite, and the format's answer to degenerate play is the DM — an adaptive, omniscient human referee. Conduct claims are vendor claims: a companion module must verify its doctrine adversarially ("just tell me the optimal round") per answer client, with dated ledger rows; nothing below verified becomes a default. Verify the default; do not pretend it is a wall.
...
- **Machine prose sells; it just carries no copyright.** Unprotectable is not unsellable — public-domain compilations change hands every day — and a campaign wiki is a compilation whose selection and arrangement are the seller's own work. So the fence discloses instead of refusing: every bundle ships an `AUTHORSHIP.md` naming who wrote each page, the share that is machine-written, and the plain fact that those pages are the buyer's to use without anyone's permission. Refusing them would have been the wrong instrument twice over — it asserts a legal claim the law does not make, and it would price a seller out of a campaign built the way this toolkit builds one. What the fence still refuses is a page that asserts a sale while naming no author at all, because a sale is exactly the moment authorship must be answered rather than assumed. What it never does is detect: see [Who holds the pen](#who-holds-the-pen).
- **Publication is a private repository, not a service.** The publisher keeps the campaign in a private Git repository and adds each buyer as a collaborator; the buyer's agent reads it from there, exactly as it reads any other campaign. Access control, revocation, and delivery are the forge's problem rather than Eddic's, which is the point: no SaaS to run, no gateway holding content, no account anyone must create beyond the one they have. It follows the same rule as everything else here — a sale is a snapshot handed over, not a tenancy — and it means the honest warning about public repositories can be stated once and simply: opening a pull request against a public repository *is* publication, which is free for a free campaign and fatal for a paid one.
