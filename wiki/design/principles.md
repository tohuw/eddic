...
- **Machine prose is never transactable.** Pure machine output carries no protectable expression, so the fence refuses a page marked for sale whose authorship is `machine`, and refuses one that asserts a sale while naming no author at all — a sale is exactly the moment the question must be answered honestly. Both refusals name the same two remedies: rewrite the page with its writer until it is honestly `mixed`, or unmark it and keep it local. The exclusion is the projection's ordinary arithmetic, so dropping a page from the sale costs nothing and loses nothing — the campaign keeps it. What the fence never does is detect: see [Who holds the pen](#who-holds-the-pen).
