...
The player Constellation is built from the projection, and that closure (a
...
1. Vendor the verb:
       ...
       uv run <campaign>/.eddic/eddic.py manifest record \
           --module constellation --version 0.1.0 --verbs graph

2. Build the player Constellation from the projection:
   ...
   `<site_dir>/constellation.html`; that file rides the existing site deploy
   and is served at `/constellation.html` by the same ASSETS binding as the
   rest of the site — no worker change, no new serving surface.

3. Rebuild the site and deploy as usual (`eddic build`, then the
   publish/retrieval deploy step). The Constellation is a static sibling of the
   rendered pages; a node's link is a page-relative `.html` path, so it
   resolves both locally and behind the deploy.

4. Optionally link the Constellation from the site (a nav or footer entry
   pointing at `/constellation.html`) so readers can find it. This is a plain
...
Constellation simply reflects whatever the wiki says now.

## Decision points

- **Which Constellation to build.** Default: the player Constellation (`--mode player`,
  from the projection) — the one safe to publish. A DM Constellation
  ...
  output `<campaign>/constellation.dm.html`, outside the served `site_dir`) and
  ...
  not built in v1 to keep the Constellation a single self-contained file and off
  `worker.js`.
- **Linking it from the site.** Default: add one unobtrusive footer or
  nav link to `/constellation.html` if the template makes that clean; otherwise
  leave the file reachable by direct URL. The Constellation works standalone
...
- **Backlinks panel.** Default: on, and there is nothing to configure.
  ...
  or request. It is firewall-correct by construction: the player Constellation is
...
- **The Party mark.** Default: on, and there is nothing to configure. If
  ...
  the wiki has no `party.md`, the Constellation is byte-identical to one built
  ...
  world map.
- **Palette.** Default: the Constellation matches the stock site shell
...
- `uv run modules/constellation/verify/run.py` — plants a fixture with a DM-only
  page and a DM->player edge and asserts: (a) node/edge extraction
  matches a golden; (b) the player-mode Constellation, built from the
  ...
  DM-mode Constellation over the master, by contrast, does see the page — proving
  the source-tree choice is the firewall.
- In the real campaign: run `eddic graph --mode player`, open
  `dist/site/constellation.html`, drag/scroll to pan and zoom, click a node and
